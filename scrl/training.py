...
def train(
    ...
    for indices in dataset_indices:
        ...
        sample_labels, sample_data) = sampling.best_of_k_samples(
            manager, tokenizer, reward_generator,
        ...
        if args.sample_aggregation == "max":
            loss = losses.compute_loss(args, probs, sample_probs, s_reward, a_reward)
        else:
            assert args.sample_aggregation == "mean"
            loss = 0.
            for sample_probs_i, s_rewards_i in zip(sample_data["probs"], sample_data["rewards"]):
                s_reward_i = np.mean(s_rewards_i)
                loss_i = losses.compute_loss(args, probs, sample_probs_i, s_reward_i, a_reward)
                loss += loss_i
